...
function creerBoutonCreneau(heure) {
  ...
  box.className =
    "slot-box block rounded-[20px] border border-div bg-default px-3 py-4 text-center font-hatton text-main transition-all duration-300 hover:shadow-xl/20 peer-checked:bg-button peer-checked:border-[#8F755E] sm:rounded-[24px] sm:px-4";
...
function afficherAucunCreneau(message) {
  ...
  texte.className =
    "col-span-full rounded-[20px] bg-default px-4 py-4 text-center font-hatton text-main";
...
function mettreAJourCreneaux() {
  ...
  fetch(
    baseUrl +
      "/auth/creneaux_rdv.php?expert_id=" +
      encodeURIComponent(expert.value) +
      "&date=" +
      encodeURIComponent(dateRdv.value),
  )
